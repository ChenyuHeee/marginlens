  ...
  if (!dbPromise) {
    dbPromise = openDB(DB_NAME, DB_VERSION, {
      upgrade(db, oldVersion) {
        ...
        if (oldVersion < 5) {
          if (!db.objectStoreNames.contains('teachingSites')) {
            db.createObjectStore('teachingSites', { keyPath: 'documentId' });
          }
        }
      },
    });
...
// Teaching Sites (generated by the 3-agent pipeline, keyed by documentId)
import type { TeachingSite } from './teaching/templates';

export async function getTeachingSite(documentId: string): Promise<TeachingSite | undefined> {
  const db = await getDB();
  return db.get('teachingSites', documentId);
}

export async function saveTeachingSite(site: TeachingSite): Promise<void> {
  const db = await getDB();
  await db.put('teachingSites', site);
}

export async function deleteTeachingSite(documentId: string): Promise<void> {
  const db = await getDB();
  await db.delete('teachingSites', documentId);
}
